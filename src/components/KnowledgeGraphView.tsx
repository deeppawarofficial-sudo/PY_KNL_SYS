        ...
        <div className="lg:col-span-4 bg-white border border-slate-200 rounded-xl p-5 shadow-2xs space-y-4">
          ...
          {selectedNode ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 text-xs font-mono font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 rounded capitalize">
                  {selectedNode.type}
                </span>
                  ...
                  {data.links
                    ...
                    .map((l, i) => (
                      <div key={i} className="bg-slate-50 p-2 rounded border border-slate-200 text-[11px] font-mono text-slate-700">
                        {l.source} ➔ <span className="text-indigo-600 font-bold">{l.relationship}</span> ➔ {l.target}
                      </div>
                    ))}
                </div>
              </div>
