...
export const SynthesisWorkspace: React.FC<SynthesisWorkspaceProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-2xs space-y-4">
        ...
        <div className="relative">
          <textarea
            ...
            rows={3}
            className="w-full bg-slate-800/50 border border-slate-800 focus:border-amber-600 focus:bg-slate-900 rounded-xl p-3.5 pr-32 text-sm text-slate-100 placeholder-slate-600 outline-none resize-none transition-all font-sans"
            onKeyDown={(e) => {
          ...
          <button
            ...
            className="absolute right-3 bottom-3.5 px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:bg-slate-700 disabled:text-slate-400 text-white font-semibold text-xs rounded-lg transition-all flex items-center gap-1.5 shadow-2xs cursor-pointer disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <>
          ...
          <div className="flex flex-wrap gap-2">
            {PRESET_QUERIES.map((preset, idx) => (
              <button
                ...
                className="group flex items-center gap-2 px-3 py-1.5 bg-slate-800/50 hover:bg-amber-50/80 border border-slate-800 hover:border-amber-300 rounded-lg text-xs text-slate-300 hover:text-amber-900 transition-all text-left cursor-pointer"
              >
                <span className="font-bold text-amber-600 group-hover:text-amber-700">[{preset.tag}]</span>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-amber-50/70 border border-amber-200 rounded px-2.5 py-1 text-amber-950 font-semibold text-xs outline-none focus:border-amber-500 cursor-pointer shadow-2xs"
              >
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-slate-800/50 border border-slate-800 rounded px-2 py-1 text-slate-200 font-mono text-xs outline-none focus:border-amber-500"
              >
      ...
      {/* Loading Banner */}
      {isLoading && (
        <div className="bg-slate-900 border border-amber-200 rounded-xl p-8 text-center space-y-4 shadow-2xs">
          <div className="w-12 h-12 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center mx-auto text-amber-600 shadow-2xs">
